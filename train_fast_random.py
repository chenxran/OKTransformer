...
class GLUETrainer:
    def __init__(self, config, logger):
        ...
        self.train_loader = DataLoader(
            self.train_dataset,
            batch_size=self.config["batch_size"],
            collate_fn=self.train_dataset.collate_fn if "t5" not in self.config["model"] else self.train_dataset.collate_fn_t5,
            shuffle=self.config["shuffle"],
        )
        ...
        self.test_loader = DataLoader(
            self.test_dataset,
            batch_size=self.config["batch_size"],
            collate_fn=self.test_dataset.collate_fn if "t5" not in self.config["model"] else self.test_dataset.collate_fn_t5,
            shuffle=self.config["shuffle"],
        )
        ...
        with torch.no_grad():
            with tqdm(total=len(self.test_loader), ncols=140) as pbar:
                ...
                for step, examples in enumerate(self.test_loader):
                    examples = examples.copy()
                    labels.extend(examples['labels'].tolist() if "t5" not in self.config["model"] else examples["label_text"])
                    outputs = self.model(examples)
                    predictions.extend(outputs['prediction'].tolist() if not isinstance(outputs["prediction"], list) else outputs["prediction"])
                ...
                if "t5" in self.config["model"]:
                    labels = [T5LABEL2LABEL[self.config["task"]][temp] for temp in labels]
                    predictions = [T5LABEL2LABEL[self.config["task"]][temp] for temp in predictions]
                evaluations = self.compute(predictions, labels)
