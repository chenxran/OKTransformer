...
T5TEMPLATE = {
    ...
    "sts-b": "stsb sentence1 {} sentence2 {}",
}
...
class SentencePairDataset(Dataset):
    ...
    def collate_fn_t5(self, batch):
        ...
        for ex in batch:
            ...
            sentence2s.append(ex["sentence2"])
            commonsenses.append(ex["commonsense"])
            labels.append(LABEL2T5LABEL[ex["label"]][self.config["task"]])
            inv_labels.append(LABEL2T5LABEL[1 - ex["label"]][self.config["task"]])

        encoding["label_text"] = labels
        encoding["inv_label_text"] = inv_labels

        inputs_sent = [T5TEMPLATE[self.config["task"]].format(batch[i]["sentence1"], batch[i]["sentence2"]) for i in range(len(batch))]
        encoding["encoding1"] = self.tokenizer(
            inputs_sent,
            padding="longest",
            truncation=True,
        ...
        merge_commonsenses, knowledge_mask = merge_list(commonsenses, self.config, None, None)
        ...
        encoded_labels[encoded_labels == 0] = -100
        encoding["labels"] = encoded_labels

        encoded_inv_labels = self.tokenizer(
            inv_labels,
            padding="longest",
            truncation=True,
            max_length=128,
        ...
        encoded_inv_labels[encoded_inv_labels == 0] = -100
        encoding["inv_labels"] = encoded_inv_labels 
...
class SingleSentenceDataset(Dataset):
    ...
    def collate_fn_t5(self, batch):
        ...
        inputs_sent = [T5TEMPLATE[self.config["task"]].format(batch[i]["sentence"]) for i in range(len(batch))]
        encoding["encoding1"] = self.tokenizer(
            inputs_sent,
            padding="longest",
            truncation=True,
