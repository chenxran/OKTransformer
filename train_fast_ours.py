...
# from data import (CoLADataset, EXPCoLADataset, EXPMRPCDataset, EXPQNLIDataset,
#                   EXPRTEDataset, EXPSST2Dataset, EXPSTSBDataset, MNLIDataset,
#                   MRPCDataset, QNLIDataset, QQPDataset, RTEDataset,
#                   SST2Dataset, STSBDataset, WinogradDataset,
#                   WinogradExpBERTDataset)
# from model import ExpBERTGLUEModel, ExpBERTModel, GLUEModel, WinogradModel
...
class WinogradTrainer:
    def __init__(self, config, logger):
        ...
        self.train_dataset = WinogradDataset(
            ...
            self.tokenizer, self.config, self.logger, self.config["trainset"],
            )
        ...
        self.test_datasets = [
            WinogradDataset(
                TASK2PATH[testset], 
                self.tokenizer, 
                self.config, self.logger, testset)
            for testset in self.testsets
        ]
        ...
        for test_loader in self.test_loaders:
            ...
            with tqdm(total=len(test_loader)) as pbar:
                for examples in test_loader:
                    test_iterator.append(examples) 
                    pbar.update(1)
            self.test_iterators.append(test_iterator)
    ...
    def train(self):
        ...
        for e in range(self.config["epochs"]):
            ...
            with tqdm(total=len(self.train_iterator), ncols=140) as pbar:
                labels = []
                predictions = []
                loss = []
    ...
    def test(self, epoch):
        self.model.eval()
        with torch.no_grad():
            for i, test_iterator in enumerate(self.test_iterators):
                with tqdm(total=len(test_iterator), ncols=140) as pbar:
                    loss = []
                    labels = []
                    predictions = []
                    for step, examples in enumerate(test_iterator):
                        examples = examples.copy()
                        labels.extend(examples["labels"].tolist())
                        outputs = self.model(examples)
                        predictions.extend(outputs["prediction"].tolist())
...
class GLUETrainer:
    def __init__(self, config, logger):
        ...
        self.train_dataset = TASK2DATASET[self.config['task']](  #TODO: 
            TASK2PATH[self.config['trainset']],
            self.tokenizer, self.config, self.logger,
            )
        set_random_seed(42)
        self.train_loader = DataLoader(
            self.train_dataset,
    ...
    def train(self):
        ...
        for e in range(self.config["epochs"]):
            ...
            with tqdm(total=len(self.train_iterator), ncols=140) as pbar:
                ...
                for step, examples in enumerate(self.train_iterator):
                    ...
                    # if self.total_steps == 500:
                        # end = time.time()
                        # wandb.log({'training time cost': end - start})
                        # wandb.finish()
                        # exit(0
            ...
            self.logger.info("Training Loss: {}".format(np.mean(loss)))
            self.logger.info("Number of Training data: {}".format(len(labels)))
           # torch.save(self.model, "model_{}.pkl".format(e))
            self.test(e)
    ...
    def test(self, epoch):
        self.model.eval()
        with torch.no_grad():
            with tqdm(total=len(self.test_iterator), ncols=140) as pbar:
                labels = []
                predictions = []
                for step, examples in enumerate(self.test_iterator):
                    examples = examples.copy()
                    labels.extend(examples["labels"].tolist())
                    outputs = self.model(examples)
                    predictions.extend(outputs["prediction"].tolist())
    ...
    #     for p, m in self.original.named_parameters():
    #         loss += torch.mean((self.model.state_dict()['sent_model.bert.' + p] - m) ** 2)
